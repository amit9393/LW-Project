...
class ResourceFunctions {
  ...
  def SplitDateOnColumn(SplitDateCol:Array[String], spark:SparkSession):Array[String] = 
  {
    ...
    val SelectQuery = splitDateString(SplitDateCol)
    val ResDF = spark.sql("SELECT *,"+SelectQuery._1+" FROM InpTable")
    ResDF.createOrReplaceTempView("InpTable")
    SelectQuery._2.split(",")
  }
  def splitDateString(SplitDateCol: Array[String]): (String, String) =
    {
      var selectOp = ""
      var ExtraColumn = ""
...
//       sb.append("ValidateLengthString done: " +i+"\n")
  ...
  def specialCharReplace(InpColumn:Array[String],SpecialCharColumn:Array[(String,String)],spark:SparkSession):Unit =
  {
    val Replace_Spec_char = ((inpStr:String, replaceChar:String) => {
      var op = inpStr
      try {
        op = inpStr.replaceAll("["+replaceChar+"]", "")
      } catch {
        case t: Throwable => op = inpStr
      }
      op
    })
   
    spark.udf.register("ReplaceSpecChar",Replace_Spec_char)
    
    val SelectStatement = specialCharSelect(InpColumn,SpecialCharColumn)
    val ResOp = spark.sql("SELECT "+SelectStatement+" FROM InpTable")
    
    ResOp.createOrReplaceTempView("InpTable")
  }
  
  def specialCharSelect(InpColumn:Array[String],SpecialCharColumn:Array[(String,String)]):String = 
  {
    var selOp = Array[String]()
    
    val SpecialCol = SpecialCharColumn.map(a => a._1)
    for(inpCol <- InpColumn)
    {
      if(SpecialCol.contains(inpCol))
      {
        val SpecialCharacters = SpecialCharColumn(SpecialCol.indexOf(inpCol))._2
        selOp = selOp.:+("ReplaceSpecChar(`"+inpCol+"`,'"+SpecialCharacters+"')")
      }
      else
      {
        selOp = selOp.:+("`"+inpCol+"`")
 ...
 def timeSpanCalculate(AggCol:Array[String], CombinationColumn:Array[String],DateColumn_Tup:(String,String),TimeSpan_StartDT_Tup:(String,String),TimeSpan_EndDT_Tup:(String,String),ResTableName : String, spark:SparkSession):Unit = 
  ...
  def timeSpan_process_temp(AggCol_Type:Array[String], CombinationColumn_Days:Array[String],CombinationColumn_Others:Array[String], DateColumn_Tup:(String,String),ParameteDate:(String,String),spark:SparkSession):Unit = 
  {
    ...
    val LTD = (("LAST_30_DAY","ddMMMyyyy"),("TIME_KEY","yyyyMMdd"),"LTD_table")
    ...
    val Quarter = (("FIRST_DAY_QUARTER","ddMMMyyyy"),"Quarter_table")
	  val YTD = (("FIRST_DAY_YEAR","ddMMMyyyy"),"YTD_table")
		...
		val TDP = (("TDP_START","ddMMMyyyy"),"TDP_table")
		val MOC = (("MOC_START_DATE","ddMMMyyyy"),"MOC_table")
		
		val Arr_TimespanType_LTD:Array[((String,String),(String,String),String)] = Array(LTD)
    
    for(timespan_type <- Arr_TimespanType_LTD)
    {
    ...
    for(timespan_type <- Arr_TimespanType)
    {
      LTM_MAT_timeSpanCalculate_temp(AggCol_Type, CombinationColumn_Others, DateColumn_Tup, timespan_type._1,timespan_type._2, ParameteDate, timespan_type._3, spark)
    }
    
    val Arr_TimespanType_others:Array[((String,String),String)] = Array(MTD,Month,QTD,YTD,Year,Quarter,TDP,MOC)
    ...
    val LTD_DF = spark.sql("SELECT * FROM LTD_table").withColumnRenamed("LAST_30_DAY", "TIME_KEY").withColumn("TIMEPERIOD_TYPE", lit("L30D"))
    val TDP_DF = spark.sql("SELECT * FROM TDP_table").withColumnRenamed("TDP_START", "TIME_KEY").withColumn("TIMEPERIOD_TYPE", lit("TDP"))
    val MOC_DF = spark.sql("SELECT * FROM MOC_table").withColumnRenamed("MOC_START_DATE", "TIME_KEY").withColumn("TIMEPERIOD_TYPE", lit("MOC"))
    ...
    LTD_DF.createOrReplaceTempView("LTD_DF_table")
    spark.sql("SELECT a.*, time.`TIMEPERIOD_TYPE_ID` FROM LTD_DF_table AS a LEFT JOIN dimTimePeriod_Table AS time ON a.TIMEPERIOD_TYPE = upper(time.`TIMEPERIOD_TYPE`)").drop("TIMEPERIOD_TYPE").createOrReplaceTempView("res_LTD_DF_table")
    
    TDP_DF.createOrReplaceTempView("TDP_DF_table")
    spark.sql("SELECT a.*, time.`TIMEPERIOD_TYPE_ID` FROM TDP_DF_table AS a LEFT JOIN dimTimePeriod_Table AS time ON a.TIMEPERIOD_TYPE = upper(time.`TIMEPERIOD_TYPE`)").drop("TIMEPERIOD_TYPE").createOrReplaceTempView("res_TDP_DF_table")
    
    MOC_DF.createOrReplaceTempView("MOC_DF_table")
    spark.sql("SELECT a.*, time.`TIMEPERIOD_TYPE_ID` FROM MOC_DF_table AS a LEFT JOIN dimTimePeriod_Table AS time ON a.TIMEPERIOD_TYPE = upper(time.`TIMEPERIOD_TYPE`)").drop("TIMEPERIOD_TYPE").createOrReplaceTempView("res_MOC_DF_table")
    
     
      
  }
  
  def LTM_MAT_timeSpanCalculate_temp(AggCol:Array[String], CombinationColumn:Array[String],DateColumn_Tup:(String,String),TimeSpan_StartDT_Tup:(String,String),TimeSpan_EndDT_Tup:(String,String),TimeSpan_TimeKey_Tup:(String,String),ResTableName : String, spark:SparkSession):Unit=
  {
    val Date_to_Timestamp = ((DateField:String, format:String) => {
      var op = 0L
      try {
    ...
    spark.udf.register("Date_to_Timestamp", Date_to_Timestamp)
     ...
     //val sqlQueryStatements = LTM_MAT_timeSpan_query_temp(AggCol, CombinationColumn, TimeSpan_StartDT_Tup)
     
     val AggCol_Final = AggCol.map(col => ("`"+col.split('-')(0).trim().replaceAll(" ", "_")+"`",col.split('-')(1).trim()))
    
     val CombinationColumn_Final = CombinationColumn.map(col => "`"+col.trim().replaceAll(" ", "_")+"`")
      
     val SelMonth = AggCol_Final.map(col => "F1."+col._1).mkString(",")+","+CombinationColumn_Final.map(col => "F1."+col).mkString(",")+","+"DateConvert(F2.`FIRST_DAY_MONTH`,'ddMMMyyyy') AS `FIRST_DAY_MONTH`"
     
     val MonthData_inp = spark.sql("SELECT "+SelMonth+" FROM FactTable AS F1 LEFT JOIN Date_Table AS F2 ON Date_to_Timestamp(F1.`"+DateColumn_Tup._1+"`,'"+DateColumn_Tup._2+"') = Date_to_Timestamp(F2.`"+TimeSpan_TimeKey_Tup._1+"`,'"+TimeSpan_TimeKey_Tup._2+"')")
  
     MonthData_inp.createOrReplaceTempView("MonthData_inp")
     
     val MonthSel2 = CombinationColumn_Final.mkString(",")+",`FIRST_DAY_MONTH` AS `TIME_KEY`,"+AggCol_Final.map(col => "SUM(CAST("+col._1+" AS "+col._2+")) AS "+col._1).mkString(",")
      
     val GroupBy = CombinationColumn_Final.mkString(",")+",`FIRST_DAY_MONTH`"
      
     val Month = spark.sql("SELECT "+MonthSel2+" FROM MonthData_inp GROUP BY "+GroupBy)
     
     Month.createOrReplaceTempView("Month_table")
  
     val InpDF = spark.sql("SELECT *,Date_to_Timestamp(`"+DateColumn_Tup._1+"`,'yyyyMMdd') AS `NUM_ProcessDate` FROM Month_table")
    
    InpDF.createOrReplaceTempView("DateConvInp_Table")

    val sqlQueryStatements1 = timeSpan_query(AggCol, CombinationColumn, TimeSpan_StartDT_Tup,TimeSpan_EndDT_Tup)
  
    //val TimespanStart_End = spark.sql("SELECT Date_to_Timestamp(`L3M_Start`,'yyyyMMdd') AS `NUM_StartDT`,Date_to_Timestamp(`L3M_End`,'yyyyMMdd') AS `NUM_EndDT`, L3M_Start FROM Date_Table").distinct()
    val TimespanStart_End = spark.sql("SELECT "+sqlQueryStatements1._1+" FROM Date_Table").distinct()
    TimespanStart_End.createOrReplaceTempView("TimespanStart_End")
    
   val Inp_join_startend = spark.sql("SELECT "+sqlQueryStatements1._2+" FROM DateConvInp_Table AS F1 LEFT JOIN TimespanStart_End AS F2 ON F1.NUM_ProcessDate BETWEEN F2.`NUM_StartDT` AND F2.`NUM_EndDT`")
    Inp_join_startend.createOrReplaceTempView("Inp_join_startend_Table")
   
    val ResDF = spark.sql("SELECT "+sqlQueryStatements1._4+" FROM Inp_join_startend_Table GROUP BY "+sqlQueryStatements1._5)
